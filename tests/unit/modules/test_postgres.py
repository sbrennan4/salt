...
class PostgresTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_db_alter_owner_recurse(self):
        with patch('salt.modules.postgres.owner_to',
    ...
    def test_group_update(self):
        with patch('salt.modules.postgres._run_psql',
                   Mock(return_value={'retcode': 0})):
            with patch('salt.modules.postgres.role_get',
    ...
    def test_user_exists(self):
        with patch('salt.modules.postgres._run_psql',
                   Mock(return_value={'retcode': 0})):
            with patch('salt.modules.postgres.version',
    ...
    def test_drop_extension2(self):
        with patch('salt.modules.postgres.installed_extensions',
                   Mock(side_effect=[{}, {}])):
            with patch('salt.modules.postgres._psql_prepare_and_run',
    ...
    def test_create_extension_newerthan(self):
        ...
        '''
        with patch('salt.modules.postgres.create_metadata',
                   Mock(side_effect=[
                       # create succeeded
        ...
        with patch('salt.modules.postgres._run_psql',
            ...
            with patch('salt.modules.postgres.psql_query',
                       Mock(return_value=[
                           ...
                           {'Name': 'sql'},
                           {'Name': 'plpgsql'}])):
                with patch('salt.modules.postgres.language_exists',
    ...
    def test_datadir_init(self):
        ...
        '''
        with patch('salt.modules.postgres._run_initdb',
                   Mock(return_value={'retcode': 0})):
            with patch('salt.modules.postgres.datadir_exists',
                       Mock(return_value=False)):
